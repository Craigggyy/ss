...
if (isset($_POST['upload_id']))
{
    ...
    move_uploaded_file($tmp, "uploads/studentid/" . $filename);
...
if (isset($_GET['delete']))
{
    if ($_SESSION['role'] == "admin")
    {
        ...
        $conn->query("DELETE FROM billing             WHERE user_id = '$deleteID'");
        $conn->query("DELETE FROM room_requests       WHERE user_id = '$deleteID'");
        $conn->query("DELETE FROM maintenance_requests WHERE user_id = '$deleteID'");
        $conn->query("DELETE FROM login_logs          WHERE user_id = '$deleteID'");
        $conn->query("DELETE FROM users               WHERE id = '$deleteID' AND role = 'user'");
        ...
        <div class="card-body">
            ...
            <div class="text-center mb-4">
                <img
                    ...
                    style="width:250px; height:160px; object-fit:cover; border-radius:10px; border:1px solid #ccc;">
            </div>
            <?php endif; ?>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <input type="file" name="studentid" class="form-control mb-3" required>

                <input type="submit" name="upload_id" value="Upload" class="btn btn-success">
                ...
                <tr>
                    ...
                    <td>
                        <?php if (!empty($row['student_id'])): ?>
                            <a href="uploads/studentid/<?php echo $row['student_id']; ?>" target="_blank">View ID</a>
                        <?php else: ?>
                            &mdash;
                        <?php endif; ?>
                    </td>
                    ...
                    <td>
                        ...
                        <?php else: ?>
                            <span class="badge-status badge-unpaid"><?php echo $row['status']; ?></span>
                        <?php endif; ?>
                    </td>

                    <td>
                        <a href="account.php?delete=<?php echo $row['id']; ?>" class="btn btn-danger btn-sm">Delete</a>
                    </td>
